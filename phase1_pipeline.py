...
class Phase1Pipeline:
    def __init__(
        ...
        collection_name: str = "neurashield_code"
    ):
        ...
        # EmbeddingGenerator will later be changed
        # to use local embeddings or Groq-compatible logic
    ...
    def run_pipeline(
        ...
        cost_estimate = self.embedding_gen.estimate_cost(
            total_tokens
        )

        print(
            f"Tokens: {total_tokens:,} | "
            f"Cost: {cost_estimate['estimated_cost_usd']}"
        )

        enriched_chunks = (
            self.embedding_gen.generate_batch_embeddings(
                all_chunks,
